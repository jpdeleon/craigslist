...
class Base:
    ...
    def get_pages(self):
        ...
        response = response = requests.get(f"https://{self.location}.craigslist.org/search/apa/apt?" 
                        + "&hasPic=1"
                        + "&availabilityMode=0"
                        + "&sale_date=all+dates"
                        )
    ...
    def scrape_craiglist(self):
        ...
        if args.verbose:
            print(f"Scraping {len(pages)} out of {len(self.get_pages())} pages.")

        for page in tqdm(pages):
            
            #get request
            response = requests.get(f"https://{self.location}.craigslist.org/search/apa/apt?" 
            ...
            for post in posts:
                ...
                if post.find('span', class_ = 'result-hood') is not None:
                    ...
                    post_price = int(post.a.text.strip().replace(self.currency_mark, ""))
    ...
    def make_df(self):
        df = pd.DataFrame({'posted': self.post_timing,
        ...
        df['number bedrooms'] = pd.to_numeric(df['number bedrooms'], errors='coerce')
    ...
    def plot_price(self, ax=None):
        ...
        if ax is None:
            fig, ax = pl.subplots(1,1, figsize=(12, 8), constrained_layout=True)
        
        sb.scatterplot(x='price', y='sqft', 
                    hue='number bedrooms', 
                    palette='viridis', 
                    x_jitter=True, 
                    y_jitter=True, 
                    s=125, 
                    data=apts[(apts.price<self.max_price) & 
                              (apts.sqft< self.max_sqft)
                    ],
                    ax=ax
                    )
    ...
    def plot_boxplot(self, ax=None):
        ...
        apts.neighborhood = apts.neighborhood.apply(lambda x: x.replace(')',''))
        apts.neighborhood = apts.neighborhood.apply(lambda x: x.replace(f', {self.location}',''))
        apts.neighborhood = apts.neighborhood.apply(lambda x: x.replace(f' and ','&'))
        apts.neighborhood = apts.neighborhood.apply(lambda x: ''.join([i for i in x if not i.isdigit()]))
        apts.neighborhood = apts.neighborhood.apply(lambda x: x.replace(f' line',''))
        apts.neighborhood = apts.neighborhood.apply(lambda x: x.replace(f' station',''))
        if self.location=="seoul":
            apts.neighborhood = apts.neighborhood.apply(lambda x: x.replace(f' stn',''))
            apts.neighborhood = apts.neighborhood.apply(lambda x: x.replace(f' subway',''))
        if self.location=="tokyo":
            apts.neighborhood = apts.neighborhood.apply(lambda x: x.replace(f' city',''))
            apts.neighborhood = apts.neighborhood.apply(lambda x: x.replace(f'-ku',''))
        ...
        apts2 = apts.groupby(by='neighborhood').filter(lambda x, min_count=min_count: len(x) >= min_count)
        sb.boxplot(x='neighborhood', 
           y='price', 
           data=apts2[(apts2.price<self.max_price) & 
...
if __name__=='__main__':
    ...
    description="check craigslist apartments")
    ...
    parser.add_argument("-p", "--show_price", action="store_true", default=False)
    parser.add_argument("-r", "--show_regression", action="store_true", default=False)
    parser.add_argument("-b", "--show_boxplot", action="store_true", default=False)
    ...
    bc = Base(location=args.location, 
